...
const COLLECTION = 'collection'; // request 的集合类型
/**
 * 构建Document
...
function buildDocument(app) {
  ...
  let { definitions, components } = contract.getDefinitions(app);

  let filepath = path.join(app.config.baseDir, swagger.dirScanner);

  // 递归获取 tags&paths
  tag_path = getTag_Path(filepath, securitys, swagger, definitions, components);
  // console.log('document:', definitions, components);
...
function getTag_Path(fileDir, securitys, swagger, definitions, components) {
  // 已存在tag集合
  let tagNames = [];
  let tags = [];
  let paths = {};
  ...
  for (let name of names) {
    ...
    if (stat.isDirectory()) {
      const subPath = getTag_Path(filepath, securitys, swagger, definitions, components);
      // 合并子目录的扫描结果
    ...
    if (stat.isFile() && ['.js', '.ts'].indexOf(path.extname(name)) !== -1) {
      ...
      for (let i = 1; i < blocks.length; i++) {
        ...
        if (routers) {
          ...
          path_method.parameters = generateRequest(blocks[i], routers, definitions, components);
 ...
 * @param {Object} components components 信息
 */
function generateRequest(block, routers, definitions, components) {
  let parameters = [];
  let requests = comment.getComment(block, _.REQUEST);
  if (requests) {
    for (let request of requests) {
      const tmpParamters = generateParameters(request, routers, definitions, components);
      parameters.push(...(Array.isArray(tmpParamters) ? tmpParamters : [tmpParamters]));
    }
  }
  return parameters;
}
...
function getContractInBody(block, definitions) {
  ...
  if (requests) {
    for (let request of requests) {
      if (request[1] === 'body' || (request[1] === 'query' && definitions.hasOwnProperty(request[2]))) {
        return request[2];
      }
    }
  }
 ...
 * * @param {Object} components components 信息
 */
function generateParameters(request, routers, definitions, components) {
  let parameter = {
    in: request[1],
  };
  const parameterType = parameter.in.toLowerCase();

  // parameters 类型
  if (parameterType === 'query' && request[2] === PARAMETERS) {
    // 使用 components
    if (components.parameters[request[3]]) {
      parameter = {
        $ref: `#/components/parameters/${request[3]}`,
      };
    }
    return parameter;
  } else if (parameterType === 'query' && request[2] === COLLECTION) {
    // 集合类型
    const collectionNames = components.parameters[request[3]];
    if (collectionNames) {
      // 将 parameter 改为数组
      parameter = collectionNames.map(v => {
        // return handleParameter(
        //   {
        //     in: v.name,
        //     $ref: `#/components/parameters/${v.name}`,
        //   },
        //   request
        // );
        return {
    ...
    return parameter;
  } else if (parameterType === 'body' && !_.itemType.includes(request[2])) {
    let schema = {};
  ...
  } else if (parameterType === 'query' && request[2].startsWith('array')) {
  ...
  if (!Array.isArray(parameter)) {
    parameter = handleParameter(parameter, request);
  }

  return parameter;
}

function handleParameter(parameter, request) {
  if (request[3]) {
