...
function generateRequest(block, routers, definitions, components) {
  ...
  if (requests) {
    for (let request of requests) {
      parameters.push(generateParameters(request, routers, definitions, components));
    }
  }
  return parameters;
}
...
function generateParameters(request, routers, definitions, components) {
  ...
  // console.log('generateParameters:', request);
  ...
  if (parameterType === 'body' && !_.itemType.includes(request[2])) {
    let schema = {};
