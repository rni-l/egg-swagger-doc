    ...
    if (stat.isFile() && ['.js', '.ts'].indexOf(path.extname(filepath)) !== -1) {
      ...
      for (let object in def) {
        CONTRACT[object] = {
          ...
          type: directory === PARAMETERS ? PARAMETERS : DEFINITIONS,
          content: def[object],
        };
      }
    }
...
function buildDefinition(source) {
  ...
  for (let object in source) {
    ...
    let dirType = source[object].type;

    if (dirType === PARAMETERS) {
      components.parameters[object] = def;
    } else {
      for (let field in def) {
        ...
        if (!type.includes(def[field].type)) {
          def[field]['$ref'] = `#/${dirType}/${def[field].type}`;
        ...
        if (def[field].type === 'array') {
          ...
          if (!itemType.includes(def[field].itemType)) {
            let itemType = { $ref: `#/${dirType}/${def[field].itemType}` };
...
function buildValidateRule(source) {
  ...
  for (let object in source) {
    let def = source[object].content;
    let dicType = source[object].type;
    if (dicType === DEFINITIONS) {
      for (let field in def) {
        if (!type.includes(def[field].type)) {
        ...
        if (def[field].hasOwnProperty('format') && def[field].format === 'date-time' && def[field].format === 'date') {
          delete def[field].format;
        }
...
module.exports = {
  ...
  getDefinitions: app => {
    ...
    return buildDefinition(JSON.parse(JSON.stringify(CONTRACT)));
  },
